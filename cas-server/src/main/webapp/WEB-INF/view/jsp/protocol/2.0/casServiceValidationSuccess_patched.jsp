<%@ page session="false" contentType="application/xml; charset=UTF-8"%><%@ taglib
...
step="1">        <cas:proxy>${fn:escapeXml(proxy.principal.id)}</cas:proxy>
    ...
    <cas:attributes><c:forEach var="attr" items="${assertion.chainedAuthentications[fn:length(assertion.chainedAuthentications)-1].principal.attributes}"
		>
      <cas:${fn:escapeXml(attr.key)}><c:forEach var="comp" items="${attr.value}" varStatus="loop"
        ><c:if test="${loop.index !=0}">,</c:if>${comp}</c:forEach></cas:${fn:escapeXml(attr.key)}></c:forEach>
    </cas:attributes>
  </cas:authenticationSuccess>
</cas:serviceResponse>